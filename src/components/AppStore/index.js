...
class AppStore extends Component {
  ...
  selectedTabItem = tabId => this.setState({activeTabId: tabId})

  updateInput = event => {
    this.setState({inputSearch: event.target.value})
  }

  filteredTbList = selectedLists => {
    const {activeTabId} = this.state
    const selectedlists = selectedLists.filter(
  ...
  filteredResults = () => {
    ...
    const selectedLists = appsList.filter(each =>
  ...
  render() {
    ...
    const selectedLists = this.filteredResults()
    const filteredResults = this.filteredTbList(selectedLists)
        ...
        <div className="">
          <ul className="app-container">
            {filteredResults.map(eachList => (
              <AppItem appList={eachList} key={eachList.appId} />
            ))}
          </ul>
        </div>
      </div>
